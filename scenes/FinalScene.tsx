...
export default function FinalScene({ replay }: { replay: () => void }) {
  ...
  return <section id="ritual" className="final-scene scene" aria-labelledby="final-title">
    ...
    <div className="final-action">{copy.cta.href === '#inicio' ? <button className="text-link" onClick={replay}>{copy.cta.label} <span aria-hidden="true">↗</span></button> : <a className="text-link" href={copy.cta.href}>{copy.cta.label} <span aria-hidden="true">↗</span></a>}<p>Un espacio para descubrir tu próximo ritual.</p></div>
  </section>;
}
